...
このリポジトリには、FREnvのβ/σ簡約全体の合流性を示すIsabelle/HOLによる形式化と、それに関連するLean 4およびMizarによる形式化を収録しています。WCTP 2026に投稿する論文に付随する検証用成果物として用意したものです。
...
**三つの形式化は、いずれも簡約全体の合流性を、それぞれ独立に証明しています。** Isabelleには、簡約全体の合流性定理と、それが依存する定義・定理・補題が含まれています。Leanの形式化は、自身の符号化に対して同じ主張を`LambdaFrenv.frenv_beta_sigma_confluent`として、Mizarの形式化は`FRENV_5:8`として証明しています。証明の道筋（補助計算系、σ正規化、σ上のβ、翻訳）は三者とも同じですが、定義と証明はそれぞれ独自のものです。符号化には依然として違いがあり（Leanは定数の型を持ち、変数の扱いも三者で異なります）、この成果物は互いの同値性が証明されているとは主張しません。
...
mizar/
  text/                 15本の記事と、監査用の記事
  dict/                 ここで導入する記号のための語彙ファイル
  verify.sh             依存関係の順にすべての記事を検証
  check.sh              記事を1本だけ検証
  docs/                 検証手順と作業の記録
  README.md             Mizarの要件、構成、証明の道筋
docs/
...
[Mizarのダウンロードページ](https://mizar.uwb.edu.pl/system/)から**Mizar Ver. 8.1.15**とそのMMLをインストールし、実行ファイルを`PATH`に追加したうえで、`MIZFILES`にMizarのライブラリディレクトリを設定してください。その後、次を実行します。

```sh
cd mizar
export MIZFILES=/usr/local/share/mizar   # インストーラが設定していない場合
./verify.sh
```

このコマンドは、15本の記事を依存関係の順に検証し、それぞれをローカルの`prel/`データベースへ書き出して後続の記事から参照できるようにしたうえで、最後に監査用の記事を検査します。監査用の記事は、主要な結果をそれぞれ書き下し、引用だけを根拠として導出します。標準のMML以外は使用せず、検証中に何もダウンロードしません。このビルドは

```text
FRENV_5:8   for V being non empty set holds FrRed(V) is confluent
```

を確立します。これはIsabelle側・Lean側の定理に対応するものです。Mizarには`sorry`がなく、記事が公理を導入する手段もないため、`.err`ファイルが空であることは、すべての推論が検査されたことを意味します。構成と証明の道筋は[Mizarの手順](mizar/README.md)を、出力の読み方と個別の結果の確認方法は[検証手順の説明](mizar/docs/verification.md)を参照してください。
...
| Mizar | 両計算体系の構文解析木による符号化、MMLのNewmanの補題によるσの停止性と合流性、σ正規形、平行β簡約の三角性質、合成両立性、Hardinの解釈法、翻訳と持ち上げ | `mizar/verify.sh`で検証済み |

三者が扱う言語にも違いがあります。Isabelleは名前として文字列を使い、基本構成要素としての定数を持ちません。Leanは変数と定数を型によってパラメーター化しています。Mizarは任意の空でない集合を変数の集合としてパラメーターにとり、Isabelleと同様に定数を持ちません。この成果物は、三つの符号化の同値性が証明されているとは主張しません。相違点は[証明の見取り図](docs/proof-map.md)に記載しています。
