...
このコマンドは、14本の記事を依存関係の順に検証し、それぞれをローカルの`prel/`データベースへ書き出して後続の記事から参照できるようにしたうえで、最後に監査用の記事を検査します。監査用の記事は、主要な結果をそれぞれ書き下し、引用だけを根拠として導出します。標準のMML以外は使用せず、検証中に何もダウンロードしません。このビルドは
